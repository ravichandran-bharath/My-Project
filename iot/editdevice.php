...
$rs=mysqli_query($con,"select * from mode where email='$email';") ;
	...
	?>
			...
			<div id="main">
				...
				<center>	<strong><font size="5px" color=:"#008ca6"> Device Details</font></strong><br>
					<div class="row">
    <form class="col s12" action="editdeviceprocess.php" method="POST">
	<?php
	$did=$_GET['did'];
	$rs=mysqli_query($con,"select * from device where id=$did");
	$result=mysqli_fetch_array($rs);
	...
	?>
      <div class="row">
        <div class="input-field col s6">
          ...
          <input  id="tid" type="text"  name="tid" value="<?php echo $_GET['did'];?>">
          <label for="tid">Device ID</label>
        </div>
		<div class="input-field col s6">
          <input  id="tip" type="text" class="validate" name="tip" value="<?php echo $result['ip'];?>">
          <label for="tip">IP Address</label>
        </div>
        
      </div>
	   <div class="row">
	   <div class="input-field col s6">
          <input id="tport" type="text" class="validate" name="tport" value="<?php echo $result['port'];?>">
          <label for="tport">Port</label>
        </div>
        <div class="input-field col s6">
          <input  id="tpin" type="text" class="validate" name="tpin" value="<?php echo $result['pin'];?>">
          <label for="tpin">PIN</label>
        </div>
		<div class="input-field col s6">
          <input  id="tdname" type="text" class="validate" name="tdname" value="<?php echo $result['name'];?>">
          <label for="tdname">Name</label>
        </div>
        
      </div>

    <p>
      <input class="with-gap" name="group2" type="radio" id="rlight" value="light" >
    ...
    <p>
      ...
      <br>
	<input type="submit" name="ndsubmit" value="SAVE" class="waves-effect waves-light btn">
	<br>
 ...
 <div class="input-field col s6">
	...
	if(type=='light')
	{
		...
		document.getElementById("rsaver").checked = false;
		document.getElementById("rfan").checked = false;
		document.getElementById("rlight").checked = true;
	...
	else if(type=='fan')
	{
		...
		document.getElementById("rsaver").checked = false;
		document.getElementById("rlight").checked = false;
		document.getElementById("rfan").checked = true;
	...
	else if(type=='saver')
	{
		document.getElementById("rfan").checked = false;
		document.getElementById("rlight").checked = false;
		document.getElementById("rsaver").checked = true;
	...
	if($_GET['message']=='1')
			...
			echo "<script type='text/javascript'>alert('Device Edited successfull!!!');</script>";
